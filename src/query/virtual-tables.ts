 ...
 *
 * Условие из последнего параметра — не украшение: оно отсекает движения
 * *до* свёртки, поэтому итог собирается только по тому, что просили
 * (issue #59). Предикат приходит готовым из интерпретатора — он умеет
 * вычислять выражения, а этот модуль знает только про движения.
 *
 * MVP-упрощения:
...
export interface VirtualQuery {
  ...
  params: BslValue[];
  /**
   * Условие из последнего параметра: отбирает движения до подведения
   * итога. Без него итог собирается по всем движениям регистра.
   */
  filter?: (movement: Row) => boolean;
}
...
export function materializeVirtual(fx: Fixture, vq: VirtualQuery): MaterializedVirtual {
  ...
  // Условие применяется здесь, до свёртки: в этом вся разница между
  // отбором в параметрах и отбором в ГДЕ (#59).
  const movements = vq.filter ? tf.rows.filter(vq.filter) : tf.rows;
