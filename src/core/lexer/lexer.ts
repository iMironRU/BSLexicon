...
export function lex(source: string): Token[] {
  ...
  while (pos < source.length) {
    ...
    // Строка (с поддержкой многострочного продолжения через |)
    //
    // Правило 1С: строка может переноситься на следующую физическую строку
    // — но там она должна начинаться с | (после необязательных пробелов
    // и табов). Между сегментами — «\n». Пример:
    //
    //   Текст = "Уважаемый клиент,
    //           |Ваш заказ принят.";
    //
    // Если после \n нет | — это настоящая незакрытая строка.
    if (ch === '"') {
      ...
      while (pos < source.length) {
        ...
        if (c === '\n') {
          // Ищем | на следующей физической строке (после пробелов/табов)
          let ni = pos + 1;
          while (ni < source.length && (source[ni] === ' ' || source[ni] === '\t')) ni += 1;
          if (ni < source.length && source[ni] === '|') {
            value += '\n';
            line += 1;
            pos = ni + 1; // сразу за |
            continue;
