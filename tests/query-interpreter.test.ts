...
describe('Ошибки', () => {
  ...
  it('срез регистра сведений → «пока не реализован» (#47)', () => {
    const r = runQuery('ВЫБРАТЬ * ИЗ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&Дата)', fx);
    expect(r.ok).toBe(false);
    if (!r.ok) {
      // Регистры сведений в целом ещё не поддерживают виртуалок
      expect(r.errors.some((e) => /Регистр|СрезПоследних|#47/i.test(e.message))).toBe(true);
    }
  });
});
